  ...
  fileType: {
    ...
    validator: (value: FileType) => {
      ...
      if (["xls", "csv"].indexOf(value) === -1) {
        console.warn(
          `V-BeakDataTable: invalid "${value}" fileType. Should be xls, or csv`
        );
        return false;
      }
  ...
  return props.beakrows
    .sort((a: any, b: any) => {
      ...
      if (a[currentSort.value] === null || b[currentSort.value] === null) {
        return (
          Number(a[currentSort.value] === null) -
          Number(b[currentSort.value] === null)
        );
      }
      if (a[currentSort.value] || a[currentSort.value] === 0) {
        if (
          typeof a[currentSort.value] === "string" ||
          a[currentSort.value] instanceof String
        ) {
            ...
            if (filters.value) {
              if (
                val
                  ?.toString()
                  .toLocaleLowerCase()
                  .indexOf(filters.value?.toLocaleLowerCase()) > -1
              ) {
                result = true;
              }
            }
...
const lastFileName = computed(() => {
  ...
  if (props.fileName.includes(".")) {
    fileName =
      fileName.substring(0, fileName.indexOf(".")) + "." + props.fileType;
  } else {
...
const rowUniqueKey = computed((rowIndex: number) => {
  return (rowIndex) => {
    ...
    result =
      result +
      (rowIndex + 1 + pageSize.value * (currentPage.value - 1)).toString();
    return result;
  };
});
    ...
    <div class="column is-12">
      ...
      <div class="table-filters">
          ...
          <V-Field v-if="isGlobalFilter" class="search">
            <V-Control icon="mdi:table-filter">
              <input
                ...
                :placeholder="t('modalmenu.filter')"
              />
            </V-Control>
          </V-Field>
        </div>
        <div class="column is-6">
          <div class="action-buttons">
            ...
            <div
              v-if="filteredData?.length > 0 && isDownloadExcel"
            ...
            >
              ...
              >
                <i
                  class="iconify"
                  data-icon="vscode-icons:file-type-excel2"
                ></i>
              </JsonExcel>
            </div>
          </div>
      ...
      <div class="beaktable-body">
        ...
        <table
          ...
          :class="[
            isStriped && 'is-striped-rows',
            isHeadColored && 'is-header-colored',
          ]"
        >
          <thead>
            <tr>
              ...
              >
                <span
                  ...
                  :class="
                    colm?.sortable ? 'data-column-sortable' : 'data-column'
                  "
                  @click="sort(colm?.field, colm?.sortable, colm?.type)"
                >
                  <i
                    v-if="sortDefault(colm?.field, colm?.sortable, colm?.type)"
                    class="lnil lnil-sort"
                  ></i>
                  <i
                    v-else-if="
                      sortIcon(colm?.field, colm?.sortable, colm?.type) &&
                      sortAsc
                    "
                    class="lnil lnil-sort-amount-asc is-active"
                  ></i>
                  <i
                    v-else-if="
                      sortIcon(colm?.field, colm?.sortable, colm?.type) &&
                      sortDesc
                    "
                    class="lnil lnil-sort-amount-dsc is-active"
                  ></i>
                  <span>
                    {{ colm?.label[locale] }}
                  </span>
          ...
          <template v-for="(row, rowidx) in sortedData" :key="rowidx">
            <tr>
              <input
                ...
                name="custId"
                :value="rowidx"
                :oninput="onChange(rowidx)"
              />
              <td
                v-if="isShowLineNumber"
                :class="
                  isShowDetail && row.subTableData?.length > 0
                    ? 'has-pointer-cursor'
                    : ''
                "
                @click="
                  isShowDetail
                    ? viewRowDetail($event, rowUniqueKey(rowidx))
                    : ''
                "
              >
                {{ rowUniqueKey(rowidx) }}
              ...
              >
                <slot :name="`item:${colm?.field}`" :item="row" :colm="colm">
                  <span
                    v-if="colm?.type === 'currency'"
                    :data-negative="false"
                    class="dark-text capital"
                    >{{
                      row[colm?.field]
                        ? displayInCurrency(
                            row[colm?.field],
                  ...
                  <span
                    ...
                    :class="[
                      row[colm?.field + 'Color']?.length > 0
                        ? 'is-' + row[colm?.field + 'Color']
                        : 'is-primary',
                    ]"
                  ...
                  <span v-else-if="colm?.type === 'file'">
                    <img
                      class="table-icon"
                      :src="
                        '/images/icons/files/' + row[colm?.field] + '-beak.svg'
                      "
                      alt=""
                    />
                  </span>
                  <span v-else class="dark-text capital">{{
                    row[colm?.field] ? row[colm?.field] : "-"
                  }}</span>
                </slot>
              </td>
              <td
                v-if="beakAction?.length > 0"
                data-action="true"
                data-title="Action"
              >
                <DataActions
            ...
            <tr
              v-show="
                isShowRowDetail === rowUniqueKey(rowidx) &&
                isShowDetail &&
            ...
            >
              <td
                :colspan="
                  beakcolumns?.length +
                  (isShowDetail ? 1 : 0) +
                  (beakAction?.length > 0 ? 1 : 0)
                "
              >
                <slot
                  :name="`subitem:rowslot`"
                  :item="row"
                  :itemkey="rowUniqueKey(rowidx)"
                >
                  <table
                  ...
                  >
                    <tr>
                      <th
                        ...
                        :style="
                          colm.type === 'colspan2'
                            ? '--data-width:' + beaksubcolumns?.length / 3
                            : '--data-width:' + beaksubcolumns?.length
                        "
                      >
                    ...
                    <tr
                      v-for="(subrow, subrowidx) in row.subTableData"
                      :key="subrowidx"
                    >
                      <td
                        v-for="(colm, subcolmidx) in beaksubcolumns"
                        :key="subcolmidx"
                      >
                        <span v-if="colm.type === 'inherit'">{{
                          row[colm?.field] ? row[colm?.field] : "-"
                        }}</span>
                        <span v-else-if="colm.type === 'datetime'">{{
                          subrow[colm.field]
                            ? convertDate(subrow[colm.field], locale)
                            : "-"
                        }}</span>
                        <span v-else class="dark-text capital">{{
                          subrow[colm.field] ? subrow[colm.field] : "-"
                        }}</span>
                      </td>
                    </tr>
                  </table>
        ...
        <span class="table-footnote">
          {{
            t("paging.showlabel", {
              start: (sortedData?.length === 0
                ? 0
                : (currentPage - 1) * pageSize + 1
              )?.toLocaleString(locale),
              end: (currentPage * pageSize > filteredData?.length
                ? filteredData?.length
